...
def uploadimage():
    if request.method == "POST":
        ...
        saved_filename = f'{temp_name}_{image.filename}'
        save_path = os.path.join('upload_images', saved_filename)
        image.save(save_path)
        print(save_path)
        prediction = model_predict(save_path)
        return render_template('home.html', result=True, imagepath = url_for('uploaded_images', filename=saved_filename), prediction = prediction)
    else:
        return redirect('/')
        return redirect('/')
